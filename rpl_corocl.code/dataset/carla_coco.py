...
from dataset.training.carla import Carla
...
class CarlaCocoMix(torch.utils.data.Dataset):
    def __init__(self, split, preprocess, carla_root='', coco_root="", cs_split=None, coco_split=None):
        ...
        self.city = Carla(root=carla_root, mode=self.cs_split)
    ...
    def __getitem__(self, idx):
        ...
        city_target = numpy.array(Image.open(self.city.targets[city_idx]).convert('RGB'), dtype=numpy.uint8)

        trainid_mask = numpy.zeros(city_target.shape[:2], dtype=numpy.long)
        for k, v in carla_color2trainId.items():
            trainid_mask[(city_target == k).all(-1)] = v
        city_target = trainid_mask
